...
public class ECKeyOutputStream extends KeyOutputStream {
  ...
  private CellBuffers chunkBuffers;
  private int cellSize = 1024;
  ...
  public ECKeyOutputStream() {
    ...
    ECSchema schema = new ECSchema("rs", 3, 2);
    ErasureCodecOptions options = new ErasureCodecOptions(schema);
    RSErasureCodec codec = new RSErasureCodec(new Configuration(), options);
  ...
  public ECKeyOutputStream(
    ...
    chunkBuffers = new CellBuffers(2, cellSize, 5, numDataBlks);
    this.config = config;
  ...
  public void write(byte[] b, int off, int len)
    ...
    int currLen = (int) Math.min(len, cellSize);
    int iters = len/cellSize;
    int lastCellSize = len%cellSize;
    while (iters > 0) {
      handleWrite(b, off, cellSize, false);
      off += cellSize;
      iters--;
    ...
    if (lastCellSize > 0) {
      handleWrite(b, off, lastCellSize, false);
      if(blockOutputStreamEntryPool.getCurrIdx()==numDataBlks){
        //Lets encode and write
        //encoder.encode();
        writeParityCells();
  ...
  void writeParityCells() throws IOException {
    final ByteBuffer[] buffers = chunkBuffers.getBuffers();
    ...
    for (int i = numDataBlks; i < numDataBlks+2; i++) {
      ...
      handleWrite(buffers[i].array(), 0, cellSize, false);
    }
    chunkBuffers.clear();
  }
  ...
  private void handleWrite(byte[] b, int off, long len, boolean retry)
    ...
    while (len > 0) {
      try {
        //check if previous previous cell was not full enough
        int maxLenthToWriteInThisIter = (int) len;
        if(len % cellSize !=0){
          maxLenthToWriteInThisIter = (int) (len % cellSize);
        }

        int pos = chunkBuffers.addTo(blockOutputStreamEntryPool.getCurrIdx(), b, off, (int)maxLenthToWriteInThisIter);
        BlockOutputStreamEntry current = blockOutputStreamEntryPool.allocateBlockIfNeeded();
        // length(len) will be in int range if the call is happening through
        // write API of blockOutputStream. Length can be in long range if it
        // comes via Exception path.
        int expectedWriteLen = Math.min((int) maxLenthToWriteInThisIter,
        ...
        // acknowledged.
        int writtenLength =
                writeToOutputStream(current, retry, maxLenthToWriteInThisIter, b, expectedWriteLen,
                off, currentPos);
        ...
        if(writtenLength == maxLenthToWriteInThisIter){
  ...
  class CellBuffers {
    ...
    CellBuffers(int numParityBlocks, int cellSize, int numAllBlocks, int numDataBlks) {
