...
Transfer Leadership for OM
    ...
    ${result} =             Execute                 ozone admin om transferleadership --service-id=omservice --transfer-om-id ${followerOM}
                            LOG                     ${result}
                            Should Contain          ${result}               Successfully       1

    ${newLeaderOM} =        Get OM Leader Node
                            Should be Equal         ${followerOM}           ${newLeaderOM}

    # Test write file after transfer leadership
    Write Test File
