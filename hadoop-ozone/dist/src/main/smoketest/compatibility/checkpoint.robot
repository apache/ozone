...
Download Checkpoint V1
    ...
    ${result} =           Execute and checkrc    curl -f -s --connect-timeout 10 --max-time 30 -o ${download_file} http://${OM_HOST}:${OM_PORT}/dbCheckpoint    ${expected_result}
...
Download Checkpoint V2
    ...
    ${result} =           Execute and checkrc    curl -f -s --connect-timeout 10 --max-time 30 -o ${download_file} http://${OM_HOST}:${OM_PORT}/v2/dbCheckpoint    ${expected_result}
...
Checkpoint V2 Endpoint Compatibility
    [Documentation]    Test v2 checkpoint endpoint (/dbCheckpointv2) - should only work with new cluster
