...
#                                Execute                    testMetadataKey=$(printf 'k%.0s' {1..20})
#                                Execute                    testMetadataValue=$(printf 'v%.0s' {1..20})
                                Execute                    testMetadataKey=$(printf 'k%.0s' {1..20});testMetadataValue=$(printf 'v%.0s' {1..20});echo "$testMetadataKey=$testMetadataValue" > /tmp/test_test.txt
...
#    ${result} =                 Execute AWSS3APICli with testFile        put-object --bucket ${BUCKET} --key ${PREFIX}/putobject/custom-metadata/key2 --body /tmp/testfile2 --metadata="$testMetadata"
...
#    ${result} =                 Execute AWSS3APICli with testFile        put-object --bucket ${BUCKET} --key ${PREFIX}/putobject/custom-metadata/key2 --body /tmp/testfile2 --metadata '{"$(printf 'k%.0s' {1..20})"="$(printf 'v%.0s' {1..20})"}'
#    ${result} =                 Execute AWSS3APICli with testFile        put-object --bucket ${BUCKET} --key ${PREFIX}/putobject/custom-metadata/key2 --body /tmp/testfile2 --metadata /tmp/test_test.txt
