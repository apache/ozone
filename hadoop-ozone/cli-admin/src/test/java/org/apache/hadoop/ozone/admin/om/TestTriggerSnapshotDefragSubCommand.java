...
public class TestTriggerSnapshotDefragSubCommand {
  ...
  public void setup() throws Exception {
    ...
    omClient = mock(OzoneManagerProtocolClientSideTranslatorPB.class);
  ...
  public void testTriggerSnapshotDefragWithWait() throws Exception {
    ...
    when(parent.createOmClient(null, null, false)).thenReturn(omClient);
    when(omClient.triggerSnapshotDefrag(false)).thenReturn(true);
    ...
    verify(omClient).triggerSnapshotDefrag(eq(false));
  ...
  public void testTriggerSnapshotDefragWithWaitFailure() throws Exception {
    ...
    when(parent.createOmClient(null, null, false)).thenReturn(omClient);
    when(omClient.triggerSnapshotDefrag(false)).thenReturn(false);
  ...
  public void testTriggerSnapshotDefragWithNoWaitOption() throws Exception {
    // Mock the client
    when(parent.createOmClient(null, null, false)).thenReturn(omClient);
    when(omClient.triggerSnapshotDefrag(true)).thenReturn(true);
    ...
    verify(omClient).triggerSnapshotDefrag(eq(true));
  ...
  public void testTriggerSnapshotDefragWithServiceId() throws Exception {
    ...
    when(parent.createOmClient("om-service-1", null, false))
        .thenReturn(omClient);
    when(omClient.triggerSnapshotDefrag(false)).thenReturn(true);
    ...
    // Verify the client was created with correct service ID
    verify(parent).createOmClient(eq("om-service-1"), eq(null), eq(false));
    verify(omClient).triggerSnapshotDefrag(eq(false));
  ...
  public void testTriggerSnapshotDefragWithHost() throws Exception {
    ...
    when(parent.createOmClient(null, "om-host-1", false))
        .thenReturn(omClient);
    when(omClient.triggerSnapshotDefrag(false)).thenReturn(true);
    ...
    // Verify the client was created with correct host
    verify(parent).createOmClient(eq(null), eq("om-host-1"), eq(false));
    verify(omClient).triggerSnapshotDefrag(eq(false));
  ...
  public void testTriggerSnapshotDefragWithIOException() throws Exception {
    ...
    when(parent.createOmClient(null, null, false)).thenReturn(omClient);
    when(omClient.triggerSnapshotDefrag(false))
