...
/**
 * Tests OperationInfo om database table for Ozone object storage operations.
 */
public class TestOperationInfo {
  ...
  private OperationInfo createOperationInfo() {
  ...
  public void testTableExists() throws Exception {
    Table<String, OperationInfo> operationInfo =
        omMetadataManager.getOperationInfoTable();
    Assertions.assertTrue(operationInfo.isEmpty());
  }
  ...
  public void testAddNewOperation() throws Exception {
    Table<String, OperationInfo> operationInfo =
        omMetadataManager.getOperationInfoTable();
    operationInfo.put(EXPECTED_OPERATION_KEY, createOperationInfo());
    Assertions.assertEquals(EXPECTED_OPERATION_ID,
        operationInfo.get(EXPECTED_OPERATION_KEY).getTrxLogIndex());
  }

  @Test
  public void testDeleteOperationInfo() throws Exception {
    Table<String, OperationInfo> operationInfo =
        omMetadataManager.getOperationInfoTable();

    Assertions.assertFalse(operationInfo.isExist(EXPECTED_OPERATION_KEY));
    operationInfo.put(EXPECTED_OPERATION_KEY, createOperationInfo());
    Assertions.assertTrue(operationInfo.isExist(EXPECTED_OPERATION_KEY));
    operationInfo.delete(EXPECTED_OPERATION_KEY);
    Assertions.assertFalse(operationInfo.isExist(EXPECTED_OPERATION_KEY));
  }
}
