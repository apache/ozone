...
/**
 * Test set object metadata request for FSO bucket.
 */
public class TestS3SetObjectMetadataRequestWithFSO extends TestS3SetObjectMetadataRequest {
  ...
  @Test
  public void testValidateAndUpdateCacheSetObjectMetadataToDir() throws Exception {
    OMRequestTestUtils.addVolumeAndBucketToDB(volumeName, bucketName, omMetadataManager, getBucketLayout());

    addKeyToTable();

    OMRequest modifiedOmRequest =
        doPreExecute(volumeName, bucketName, PARENT_DIR, getMetadata(2), getTags(0));
  ...
  protected S3SetObjectMetadataRequest getSetObjectMetadataRequest(OMRequest originalRequest) {
    return new S3SetObjectMetadataRequestWithFSO(originalRequest, getBucketLayout());
  }
}
