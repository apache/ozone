...
public final class STSSecurityUtil {
   ...
   * {@code [0-9A-Z]} (generated by {@code S3AssumeRoleRequest} and enforced on the revoke path by
   * {@code S3STSUtils.validateTempAccessKeyId}), so it can never contain the {@code |} delimiter. Placing it
   * first therefore makes the mapping from {@code (tempAccessKeyId, originalAccessKeyId)} to key workable
   * without any escaping, even though {@code originalAccessKeyId} is unconstrained and may itself contain
   * {@code |}.
   */
