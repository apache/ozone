 ...
 */
public class OperationInfoCleanupService extends BackgroundService {
  private static final Logger LOG =
      LoggerFactory.getLogger(OperationInfoCleanupService.class);
  ...
  public OperationInfoCleanupService(long interval, TimeUnit unit, long timeout,
                               OzoneManager ozoneManager,
                               OzoneConfiguration conf) {
    super("OperationInfoCleanupService", interval, unit,
  ...
  public BackgroundTaskQueue getTasks() {
    ...
    queue.add(new OperationInfoCleanupTask());
    return queue;
  }
  ...
  private class OperationInfoCleanupTask implements BackgroundTask {
    ...
    public BackgroundTaskResult call() throws Exception {
      ...
      LOG.info("############### Running OperationInfoCleanupTask");
      ...
      try {
        estimatedKeyCount = metadataManager.countRowsInTable(
            metadataManager.getOperationInfoTable());

        LOG.info("cleanupOperationInfoIfNecessary - estimatedKeyCount={}, maxKeys={}", estimatedKeyCount, MAX_KEYS);
        if (estimatedKeyCount <= MAX_KEYS) {
          LOG.info("cleanupOperationInfoIfNecessary - nothing to do");
          return BackgroundTaskResult.EmptyTaskResult.newResult();
        }
      } catch (IOException e) {
        // XXX
      ...
      try (TableIterator<String, ? extends Table.KeyValue<String, OperationInfo>>
               tableIterator = metadataManager.getOperationInfoTable().iterator()) {
        ...
        LOG.info("cleanupOperationInfoIfNecessary - actualKeyCount={}, maxKeys={}, toDelete={}", actualKeyCount, MAX_KEYS, toDelete);
        ...
        while (tableIterator.hasNext() && toDelete > 0) {
          ...
          if (firstKeyToDelete == null) {
            firstKeyToDelete = operationInfoRow.getKey();
          }

          lastKeyToDelete = operationInfoRow.getKey();
          toDelete--;
        }

        LOG.info("cleanupOperationInfoIfNecessary - firstKeyToDelete={}, lastKeyToDelete={}",
            firstKeyToDelete, lastKeyToDelete);

        // XXX: do we need a lock here?
        if (Objects.equals(firstKeyToDelete, lastKeyToDelete)) {
          metadataManager.getOperationInfoTable().delete(firstKeyToDelete);
        } else {
          metadataManager.getOperationInfoTable().deleteRange(firstKeyToDelete, lastKeyToDelete);
        }

        long newEstimatedKeyCount = metadataManager.countRowsInTable(
            metadataManager.getOperationInfoTable());
        LOG.info("cleanupOperationInfoIfNecessary - newEstimatedKeyCount={}", newEstimatedKeyCount);
