...
public class OzoneManagerStateMachine extends BaseStateMachine {
  ...
  private final OzoneManagerSuccessfulOperationHandler successfulOperationHandler;
  ...
  public OzoneManagerStateMachine(OzoneManagerRatisServer ratisServer,
    ...
    this.successfulOperationHandler = new OzoneManagerSuccessfulOperationHandler(ozoneManager);
  }
  ...
  private Message processResponse(OMRequest request, OMResponse omResponse, TermIndex termIndex) {
    if (!omResponse.getSuccess()) {
      ...
      //
      // TODO: in 1.4.x we had access to trxLogIndex in applyTransaction
      // but it is no longer present.  The term index seems like the
      // wrong choice as a replacement but but I'm using it for now.
      successfulOperationHandler.handle(termIndex.getIndex(), request);
    }
