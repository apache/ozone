...
public class DataChecksum implements Checksum {
    ...
    // JAVA_SPEC_VER and isJavaVersionAtLeast() are copied from Hadoop3's Shell class
    // "1.8"->8, "9"->9, "10"->10
    ...
    public static DataChecksum newDataChecksum(byte[] bytes, int offset)
        ...
        if (offset < 0 || bytes.length < offset + getChecksumHeaderSize()) {
            throw new OzoneChecksumException("Could not create DataChecksum "
    ...
    private static Type mapByteToChecksumType(int type)
        ...
        try {
            ...
            throw new OzoneChecksumException("The value "+type+" does not map"+
