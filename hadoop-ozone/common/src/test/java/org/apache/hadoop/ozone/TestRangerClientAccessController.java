...
//    RangerAccessPolicy testp = new RangerAccessPolicy("old policy");
//    OzoneObj obj =
//        new OzoneObjInfo.Builder().setResType(OzoneObj.ResourceType.VOLUME).setStoreType(OzoneObj.StoreType.OZONE).setVolumeName("foovol").build();
//    testp.addAccessPolicyElem(obj, new BasicUserPrincipal("old name"),
//        IAccessAuthorizer.ACLType.READ_ACL
//        , AccessPolicy.AccessGrantType.ALLOW);
//    System.out.println(testp.serializePolicyToJsonString());
//    Assert.fail();
...
//        Collections.singletonList(new BasicUserPrincipal("hdfs")));
    ...
    // TODO: Check policy create with missing bucket/key

    Policy policy1 = new Policy(prefix + "policy1-new", prefix + "volume3");
    policy1.addBucket(prefix + "bucket1");
    policy1.addKey("*");
    policy1.setDescription(prefix + "policy1 description");
    policy1.addRoleAcl("foorole",
        MultiTenantAccessController.Acl.allow(IAccessAuthorizer.ACLType.READ_ACL));
    policy1.addRoleAcl("foorole",
        MultiTenantAccessController.Acl.deny(IAccessAuthorizer.ACLType.WRITE_ACL));
