...
public class TestNSSummaryEndpointWithFSO {
  ...
  public void testOrphanKeys() throws IOException {
    try (TableIterator<Long, ? extends Table.KeyValue<Long, OrphanKeyMetaData>>
      ...
      Map<Long, Set<Long>> parentToOrphanChildsMap = new HashMap();
      while (orphanTableIter.hasNext()) {
        ...
        parentToOrphanChildsMap.put(key, objectIds);
      }
      assertTrue(parentToOrphanChildsMap.containsKey(22L));
      assertEquals(parentToOrphanChildsMap.get(22L).size(), 1);
      assertEquals(
          parentToOrphanChildsMap.get(22L).stream().findFirst()
              .get().longValue(), 23L);
      NSSummary dirNSSummary = reconNamespaceSummaryManager.getNSSummary(23L);
      assertEquals(dirNSSummary.getSizeOfFiles(), 3074);
      assertEquals(dirNSSummary.getDirName(), "dir7");
      assertEquals(dirNSSummary.getNumOfFiles(), 2);
      assertEquals(dirNSSummary.getChildDir().stream()
          .findFirst().get().longValue(), 24L);
