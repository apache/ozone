...
public class ReconHttpClientTest {
  ...
  public void testSendPost() throws Exception {
    ...
    PowerMockito.when(HttpClients.custom()).thenAnswer((Answer)
        invocation -> httpClientBuilder);
    when(httpClientBuilder.setSSLHostnameVerifier(
        NoopHostnameVerifier.INSTANCE)).thenReturn(httpClientBuilder);
    when(httpClientBuilder.setSSLContext(any(SSLContext.class)))
        .thenReturn(httpClientBuilder);

    when(httpClientBuilder.setDefaultAuthSchemeRegistry(any(Lookup.class)))
        .thenReturn(httpClientBuilder);
    when(httpClientBuilder.build()).thenReturn(httpClient);

    when(httpClient.execute(any(HttpPost.class), any(HttpClientContext.class)))
        .thenReturn(response);
    PowerMockito.when(EntityUtils.toString(response.getEntity()))
    ...
    assertFalse(ArrayUtils.isEmpty(entities));
    assertTrue(entities.length > 0);
    assertEquals(2324814, facets.getAsJsonObject().get("count").getAsInt());
    assertEquals(100, entities.length);
    // Verify the results
    assertEquals(155074, entities[0].getReadAccessCount());
  }
  ...
  public void testSendRequest() throws Exception {
    ...
    final HttpRequestWrapper httpRequestWrapper =
        new HttpRequestWrapper("localhost", 8995,
            "/solr/ranger_audits/query", Arrays.asList(),
            HttpRequestWrapper.HttpReqType.GET);
