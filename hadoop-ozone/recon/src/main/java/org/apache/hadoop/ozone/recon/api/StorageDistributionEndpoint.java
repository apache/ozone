  ...
  public Response downloadDataNodeStorageDistribution() {
    ...
    List<DataNodeStoragePendingDeletionView> data = pendingDeletionMetrics.stream()
        .map(metric -> {
          ...
          return new DataNodeStoragePendingDeletionView(metric, report);
        })
    ...
    List<Function<DataNodeStoragePendingDeletionView, Object>> columns =
        Arrays.asList(
            v -> v.getMetric().getHostName(),
            v -> v.getMetric().getDatanodeUuid(),
            v -> v.getReport().getFilesystemCapacity(),
            v -> v.getReport().getFilesystemUsed(),
            v -> v.getReport().getFilesystemAvailable(),
            v -> v.getReport().getCapacity(),
            v -> v.getReport().getUsed(),
            v -> v.getReport().getRemaining(),
            v -> v.getReport().getCommitted(),
            v -> v.getReport().getReserved(),
            v -> v.getReport().getMinimumFreeSpace(),
            v -> v.getMetric().getPendingBlockSize()
        );
