      ...
      <div className='missing-containers-container'>
        ...
        <div className='content-div'>
          <Tabs defaultActiveKey='1'>
            <Tabs.TabPane
              key='1'
              tab={`Missing${(missingDataSource?.length > 0) ? ` (${missingDataSource.length})` : ''}`}>
              {generateTable(missingDataSource)}
            </Tabs.TabPane>
            <Tabs.TabPane
              key='2'
              tab={`Under-Replicated${(underReplicatedDataSource?.length > 0) ? ` (${underReplicatedDataSource.length})` : ''}`}>
              {generateTable(underReplicatedDataSource)}
            </Tabs.TabPane>
            <Tabs.TabPane
              key='3'
              tab={`Over-Replicated${(overReplicatedDataSource?.length > 0) ? ` (${overReplicatedDataSource.length})` : ''}`}>
              {generateTable(overReplicatedDataSource)}
            </Tabs.TabPane>
            <Tabs.TabPane
              key='4'
              tab={`Mis-Replicated${(misReplicatedDataSource?.length > 0) ? ` (${misReplicatedDataSource.length})` : ''}`}>
