...
public class OMEventListenerKafkaPublisher implements OMEventListener {
  ...
  private static final int COMPLETED_OPERATION_CONSUMER_CORE_POOL_SIZE = 1;
  ...
  public void initialize(OzoneConfiguration conf, OMEventListenerPluginContext pluginContext) {
    ...
    this.seekPosition = OMEventListenerLedgerPollerSeekPosition.create(conf);

    this.ledgerPoller = new OMEventListenerLedgerPoller(
          kafkaServiceInterval, TimeUnit.MILLISECONDS,
          COMPLETED_OPERATION_CONSUMER_CORE_POOL_SIZE,
  ...
  public void handleOperation(OperationInfo operationInfo) {
    List<String> eventsToSend = notificationStrategy.determineEventsForOperation(operationInfo);
    ...
    seekPosition.set(operationInfo.getDbKey());
  }
