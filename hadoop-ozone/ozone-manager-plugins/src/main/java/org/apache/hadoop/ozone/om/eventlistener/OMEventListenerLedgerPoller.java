...
public class OMEventListenerLedgerPoller extends BackgroundService {
  ...
  private final Consumer<OperationInfo> callback;
  ...
  public OMEventListenerLedgerPoller(long interval, TimeUnit unit,
      ...
      Consumer<OperationInfo> callback) {
  ...
  public BackgroundTaskQueue getTasks() {
    ...
    queue.add(new OMEventListenerLedgerPoller.OperationInfoConsumerTask());
    return queue;
  }
  ...
  private class OperationInfoConsumerTask implements BackgroundTask {
    ...
    public BackgroundTaskResult call() {
      if (shouldRun()) {
        ...
        try {
          for (OperationInfo operationInfo : pluginContext.listOperationInfo(seekPosition.get(), MAX_RESULTS)) {
            callback.accept(operationInfo);
          }
        } catch (IOException e) {
