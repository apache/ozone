...
/**
 * Tests TransactionInfoRepairSCM.
 */
public class TestTransactionInfoRepairSCM {
  ...
  public void testUpdateTransactionInfoTableSuccessful() {
    ...
    assertThat(out.getOutput())
        .contains(
            String.format("The original highest scm transaction info was (t:%s, i:%s)", TEST_TERM, TEST_INDEX),
            String.format("The highest scm transaction info has been updated to: (t:%s, i:%s)", TEST_TERM, TEST_INDEX)
        );
  }
  ...
  public void testCommandWhenTableNotInDBForGivenPath() {
    ...
    assertThat(err.getOutput())
            .contains(TRANSACTIONINFO.getName() + " is not in a column family in DB for the given path");
  }
