...
public class TestLDBCli {
  ...
  void testScanWithRecordsPerFile() throws IOException {
    ...
    List<String> completeScanArgs1 = new ArrayList<>(Arrays.asList(
        "--db", dbStore.getDbLocation().getAbsolutePath(),
        "scan",
        "--column-family", KEY_TABLE, "--out", scanDir1 + "/key",
    ...
    // Number of files generated by traversal
    int count1 = 0;
    File tmpDir1 = new File(scanDir1);
    if (tmpDir1.isDirectory() && tmpDir1.listFiles() != null) {
      for (File tmpFile : tmpDir1.listFiles()) {
        if (tmpFile.isFile() && tmpFile.getName().startsWith("key")) {
          count1++;
        }
      }
    }
    ...
    List<String> completeScanArgs2 = new ArrayList<>(Arrays.asList(
        "--db", dbStore.getDbLocation().getAbsolutePath(),
        "scan",
        "--column-family", KEY_TABLE, "--out", scanDir2 + "/key",
    ...
    if (tmpDir2.isDirectory() && tmpDir2.listFiles() != null) {
      for (File tmpFile : tmpDir2.listFiles()) {
        if (tmpFile.isFile() && tmpFile.getName().startsWith("key")) {
          count2++;
        }
      }
