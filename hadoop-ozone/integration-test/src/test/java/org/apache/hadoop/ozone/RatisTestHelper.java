...
public interface RatisTestHelper {
  ...
  static void initRatisConf(RpcType rpc, OzoneConfiguration conf) {
    conf.setBoolean(OzoneConfigKeys.OZONE_CONTAINER_RATIS_ENABLED_KEY, true);
    conf.set(OzoneConfigKeys.OZONE_CONTAINER_RATIS_RPC_TYPE_KEY, rpc.name());
    conf.setTimeDuration(HDDS_CONTAINER_REPORT_INTERVAL, 1, TimeUnit.SECONDS);
    conf.setTimeDuration(OZONE_SCM_STALENODE_INTERVAL, 30, TimeUnit.SECONDS);
    LOG.info("{} = {}", OzoneConfigKeys.OZONE_CONTAINER_RATIS_RPC_TYPE_KEY,
            rpc.name());
  }
