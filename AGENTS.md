...
Apache Ozone — a scalable, distributed object/file store. This is a multi-module Maven project; root coordinates and current version are in [`pom.xml`](./pom.xml) (`groupId` / `artifactId` / `version`).

**Tech stack**: Java 8 source/target with JDK21 runtime compatibility (see the `[21,]` JDK profile in `pom.xml`), built with Maven, tested with JUnit 5. Inter-process communication uses Hadoop RPC and gRPC, both over Protobuf. Persistent storage is RocksDB; replicated state is managed via Apache Ratis (RAFT).

Two top-level Maven aggregators:
...
For iterative local development, default to `-DskipShade -DskipRecon -DskipDocs` to keep build times manageable; drop the relevant flag only when you actually need to exercise that area (filesystem tests → drop `-DskipShade`; Recon UI → drop `-DskipRecon`; docs → drop `-DskipDocs`).
