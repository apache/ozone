...
public final class MetricUtil {
  ...
  /**
   * Qualifies a metrics-source name with a per-instance component so several
   * instances can register in one JVM (see
   * {@code HddsConfigKeys.HDDS_METRICS_SOURCE_DISAMBIGUATION_ENABLED}).
   * Returns {@code base} unchanged when {@code component} is null or empty,
   * so standalone services keep their original registration names.
   */
  ...
  /**
   * Returns {@code component} when metrics-source disambiguation is enabled
   * in {@code conf}, otherwise null; intended as the argument for
   * {@link #qualifySourceName(String, String)}-based factory methods.
   */
