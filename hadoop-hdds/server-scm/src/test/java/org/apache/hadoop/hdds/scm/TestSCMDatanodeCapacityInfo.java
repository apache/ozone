...
class TestSCMDatanodeCapacityInfo {
  ...
  void testNotEnoughSpaceInDataVolumes() {
    ...
    assertThat(message).startsWith("Datanode " + datanodeDetails.getUuidString() +
        " has no volumes with enough space to allocate 1024 bytes for data.");
  }
  ...
  void testNotEnoughSpaceInMetaVolumes() {
    ...
    String message = info.getInsufficientSpaceMessage();
    assertThat(message).startsWith("Datanode " + datanodeDetails.getUuidString() +
        " has no volumes with enough space to allocate 512 bytes for metadata.");
  }
  ...
  void testEnoughSpaceFound() {
    ...
    String message = info.getInsufficientSpaceMessage();
    assertThat(message).isEqualTo("Datanode " + datanodeDetails.getUuidString() +
        " has sufficient space (data: 1024 bytes required, metadata: 512 bytes required)");
  }
