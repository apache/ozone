...
public class ECReconstructionCoordinatorTask implements Runnable {
  ...
  public ECReconstructionCoordinatorTask(
      ECReconstructionCoordinator reconstructionCoordinator,
      ECReconstructionCommandInfo reconstructionCommandInfo) {
    this.reconstructionCoordinator = reconstructionCoordinator;
  ...
  public void run() {
    ...
    SortedMap<Integer, DatanodeDetails> sourceNodeMap =
        reconstructionCommandInfo.getSources().stream()
            .collect(Collectors.toMap(
                DatanodeDetailsAndReplicaIndex::getReplicaIndex,
                DatanodeDetailsAndReplicaIndex::getDnDetails,
                (v1, v2) -> v1, TreeMap::new));
    SortedMap<Integer, DatanodeDetails> targetNodeMap = IntStream.range(0,
            reconstructionCommandInfo.getTargetDatanodes().size())
        .boxed().collect(Collectors.toMap(
            i -> (int) reconstructionCommandInfo
                .getMissingContainerIndexes()[i],
            i -> reconstructionCommandInfo
                .getTargetDatanodes().get(i),
            (v1, v2) -> v1, TreeMap::new));
