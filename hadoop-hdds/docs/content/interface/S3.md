...
| ✅ [CopyObject](https://docs.aws.amazon.com/AmazonS3/latest/API/API_CopyObject.html)       | Copies an object (intra- or inter-bucket). | Supports metadata directives (COPY/REPLACE). Copying an object onto itself with metadata directive REPLACE updates the metadata in place without rewriting the object data, matching AWS behavior. |
