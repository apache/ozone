...
* fileSize (可选)
    ...
    累计阈值。正数值如果不等于分桶上界，将回传空结果。第一个分桶的上界为 1024 字节；例如，上界为 2048
    的分桶包含大于 1024 字节且不超过 2048 字节的文件。该筛选条件可以单独使用，也可以与卷和/或桶一起使用。
    
**回传**

回传不同文件范围内的文件计数，其中响应对象中的`fileSize`是对应直方图分桶的上界。每个计数仅属于该分桶，
并不是文件大小小于或等于`fileSize`的累计计数。
