...
在较高的层次上，Recon 收集和汇总来自 Ozone Manager（OM）收集和汇总元数据、Storage Container Manager（SCM）和数据节点（DN）的元数据，并充当中央管理和监视控制台。Ozone 管理员可以使用 Recon 查询系统的当前状态，而不会使 OM  或 SCM 过载。
