...
* **Hard link upper limit:** RocksDB checkpoint is implemented using file system hard links, which has an upper limit of 65,535 hardlinks per file. This means that if you have more than 65,535 snapshots for a bucket, you will not be able to create new snapshots. This is a limitation of the underlying filesystem and not specific to Ozone. Administrators should monitor the number of snapshots and consider deleting old ones if approaching this limit.
