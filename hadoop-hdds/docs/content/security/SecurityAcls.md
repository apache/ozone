...
- **S3 ACLs**: Currently, only S3 Bucket ACL is implemented in Ozone (a beta feature). S3 Object ACL is not yet implemented. Any `GetObjectAcl` or `PutObjectAcl` request will result in a `501: Not Implemented` response code.
